...
void sssp_Hybrid_CPU(int threadId,
    ...
    for (int i = start; i < end; i++) {
        ...
        if (dist[source] + weight < dist[end]) { 
            ...
            *finished = false;
            ...
            msgToDeviceNodeId[threadMsgIndex] = end;
            msgToDeviceDist[threadMsgIndex] = dist[end];
            threadMsgIndex++;
    ...
    if (threadId != (numThreads - 2)) {
        // Previous threads fill up with uint(-1)
        for (int i = threadMsgIndex; i < end; i++) {
            msgToDeviceNodeId[i] = uint(-1);
        }
        if (numThreads == 2) {
            *msgToDeviceIndex = threadMsgIndex;
        }
    } else {
        *msgToDeviceIndex = threadMsgIndex;
    }
...
__global__ void sssp_Hybrid_GPU_Process_Message(int numMsgPerThread,
    ...
    for (int i = start; i < end; i++) {
        ...
        if (nodeId != uint(-1) && dist[nodeId] > updateDist) {
