...
__device__ __host__ int findEdge(Vertex u, Vertex v, Edge *edges, int *weights)
{
	...
	for(i = 0; i < E; i++)
	{
		...
		if(edges[i].source == u.id && edges[i].target == v.id)
		{
			return weights[i];
		}
	}
...
int main(void)
{
	...
	for(i = 0; i < V;i++)
	{
		...
		if(vertices[i].id != root.id)
		{
			len[(int)vertices[i].id] = findEdge(root, vertices[i], edges, weights);
			updateLength[vertices[i].id] = len[(int)vertices[i].id];
